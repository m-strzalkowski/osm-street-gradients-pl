...
4. Pobieramy NMPT z usługi WCS:
    * możemy pobrać tylko image/x-aaigrid (ARC/INFO ASCII GRID)
        - to duże pliki, a serwer zwalnia wraz rozmiarem, więc pobieramy w kilometrowych kawałkach
...
6. Tworzymy zmaterializowany widok nachyleń:
    * ST_Segmentize - kroimy ulice na segmenty 50m
...
Aby uzyskać metadane i obsługiwane parametry, wysłac zapytanie na taki zdres z parametrem REQUEST=GetCapabilities, np.
...
1. Obsługiwane układy układy wysokościowe
    (Trzeba podać układ, bo inaczej dostaniemy błąd)
    * DTM_PL-KRON86-NH
    * DTM_PL-EVRF2007-NH
...
Format x-aaigrid (ARC/INFO ASCII GRID) jest najprostszy.
```
ncols        500
nrows        500
...
Wydaje się, że to nie problem, przecież serwer ogłosił, że obsuguje lepsze formaty, skompresowane, nie zużywające tyle transferu - GeoTiff, jpeg, png.
...
Wygląda to jakby kazdy wewnątrzny kafalek na serwerze został źle zrzutowany przy generowaniu obrazu. Pobieżny przegląd internetu sugeruje, że zwyczajnie serwer geoportalu (być może GeoServer, lub pochodna) jest domyślnie skonfigurowany aby generować obrazki, jednak robi to źle.

Ofijalna dokumentacja (https://www.geoportal.gov.pl/pl/usluga/uslugi-pobierania-wcs/) opisuje adres jednie jako `	Numeryczny Model Pokrycia Terenu – Arc/Info ASCII Grid`, nie wspomina o obrazkach, które są w xmlu chyba przez przypadek.
